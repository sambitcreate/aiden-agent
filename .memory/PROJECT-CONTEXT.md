...
- Model metadata follows a strict offline precedence contract. LM Studio and Ollama discovery use their native model endpoints, persist chat-only local names/capabilities/context/parameters/format, and take precedence for local providers. Hosted models prefer the release-bundled Artificial Analysis snapshot; models.dev fills unmatched models and fields. Provider-facing IDs never change, and unknown values stay unknown instead of being treated as unsupported. The running app never contacts either public catalog.
...
- Spatial ranking consumes fixed-snapshot Artificial Analysis Intelligence Index and median end-to-end response-time percentiles through `ModelRanking`, with visible Artificial Analysis attribution. Local models never inherit hosted benchmark placement; recognizable local variants remain Estimated and unknown entries remain Unranked. The checked-in snapshot may remain empty until an authorized Pro/Commercial refresh is run with explicit redistribution-rights confirmation.
...
- Hosted model calls, cloud transcription, Exa, remote MCP servers, and model downloads require network access and share the minimum data needed for that request. Public model-catalog refreshes happen only through the explicit development refresh or the guarded release build step.
...
- Model metadata precedence: native LM Studio/Ollama discovery, local persistence, local → Artificial Analysis → models.dev merging, fixed benchmark placement, visible attribution, dual-snapshot packaging, fail-closed refresh policy, embedding filtering, and unknown-capability behavior are covered by the 150-test suite. Type-check, lint, production renderer/main/preload build, signed unpacked package, direct `app.asar` snapshot inspection, targeted formatting, refresh preflight, and diff checks pass. The checked-in Artificial Analysis payload remains an empty validated placeholder until an authorized API refresh is run.
