...
2026-09-26 merge with main: main added `runCommandEnv(options, environment)` for the device `agent-device` shim `pathPrefix`, but no general GUI PATH handling (only an npm login-shell lookup in `device-toolchain.ts` and a fixed `gh` binary list in `github-pull-request.ts`). `run_command` now composes both: `runCommandEnv(options, agentCommandEnvironment())`, so the shim prefix stays first and the macOS CLI directories are appended after the inherited PATH.
