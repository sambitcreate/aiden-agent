...
- A logical message mounts at most 50 rich preview triggers. A shared message budget preserves that bound across the streaming reveal's multiple Markdown trees. During handoff, only the persisted assistant ID returned with the current completed response yields rich previews to the streaming copy; equal text alone never suppresses an older message.
