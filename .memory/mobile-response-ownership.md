...
## Android cross-owner GET admission checkpoint

Added a real two-ViewModel held-HTTP regression: the first owner's initial GET is held; a second owner restores and settles the stream; releasing the first GET previously replaced its presentation and the shared cached transcript. The regression fails before this slice (/tmp/aiden-response-two-owner-baseline.log; exact test diff /tmp/aiden-response-two-owner-baseline.patch). This executed owner-level negative is against d93767df, not an executed original-main owner build; the earlier cache-boundary baseline remains the original-main evidence.

Initial, terminal reconciliation and pending-title GETs now reserve shared tickets before HTTP. Responses are admitted before presentation; a rejected response adopts the namespaced accepted winner. The cache retains the winner before disk IO and rejects disk fallback after removal/purge, so ordinary persistence failure does not strand the other valid destination and deletion is not mistaken for a successful reconciliation. POST receipts and list response admission remain pending and are not covered by this claim.

Both ordinary and blocked-disk two-owner regressions pass. Full Android validation:182 JVM tests,0failures/errors/skips, lintDebug and compileDebugAndroidTestKotlin pass (/tmp/aiden-response-get-android.log). No iOS source changed in these two Android slices. Physical tests remain unexecuted. PR217 remains untouched at0cba92f106; verify/Android/Pullfrog green, Electron E2E still executing,12resolved threads last checked.

Both independent GPT-5.6 Sol medium reviews cleared the actual GET-admission diff with no introduced blocker. Review limits: pending-title retry shares the ticket/helper implementation but has no separate held-title HTTP regression yet; POST/list and later GETs receiving a stale server snapshot are not claimed fixed.

## Android Home response admission in progress

Held production Home initial refresh/pagination responses now reproduce a stale-summary overwrite on b30122002 (/tmp/aiden-summary-owner-baseline.log). The current patch reserves before HTTP, shares summary high-water with detail/removal/purge and immediate Home acceptance, rejects stale writes before disk/flows, and preserves cursor/boundary until admission. Four held-owner schedules cover initial/page vs detail/purge and successful same-cursor retry. Full183 Android JVM tests/lint/instrumentation compilation pass (/tmp/aiden-summary-owner-android.log). Both independent Sol reviews are checking the actual diff, including queued detail acceptance after a newer summary page. This slice remains uncommitted until that last ordering review is clear. Coordinator requests publication as a cohesive Android-only draft PR so hosted checks can run while remaining POST/iOS scopes continue.

Native integration owner checked the iPhone once at coordinator request: passcodeRequired=true, unlockedSinceBoot=true. No slot claimed or launch attempted; physical acceptance remains blocked by the lock.


Review corrections for the Home slice: summary request ownership is now registered synchronously before launching each HTTP request. A queued detail write can persist its independently admitted transcript, but may project a summary only while its original ticket still owns summary admission; it no longer mints a newer summary token. Once a page has committed, a later valid detail/activity write does not retroactively reject it and strand the old cursor. Context and purge retention remain checked. Added old-detail/new-page and newer-request/old-writer cache regressions, plus a real Home test that uses its injected dispatcher to mutate immediately after disk commit and before the queued UI continuation. Full184 JVM tests/lint/instrumentation compilation pass (/tmp/aiden-summary-owner-android.log). No new iOS or server contract changes in this Android-only slice. POST receipts, iOS rename/Bot response ownership and physical acceptance remain explicitly open.

Both independent GPT-5.6 Sol medium re-reviews cleared the corrected Home slice and its interactions with the two prior Android commits. Preparing a stacked draft PR on the latest PR217 base to start hosted validation; remaining POST and iOS response ownership is still assigned.
