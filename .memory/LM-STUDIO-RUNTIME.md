...
- The same fresh Pi agent now uses a model-aware `transformContext` for local and hosted providers. It keeps Pi's measured provider prefix anchored while estimating later tool results separately, reserves response/safety capacity, retains recent completed tool evidence without encouraging repeat calls, and compacts only the outbound request context. An unrepresentable active turn becomes a bounded non-tool recovery notice, while a static prompt/tool set that cannot fit fails before provider I/O; saved chats remain untouched.
- Pi-normalized `thinking_delta` events are exposed only for the explicit `lmstudio` and `ollama` presets. The main generation owner streams and persists that text on assistant messages; renderer-authored writes and redacted terminal thinking are rejected.
