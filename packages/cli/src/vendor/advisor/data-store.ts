/*
Vendored copy of main/services/data-store.ts for the Aiden CLI. The desktop
platform facade (../platform.js → electron) is replaced with the local
electron-stub; the CLI always constructs stores with an explicit root.
See advisor-runtime.ts's header for the drift test.
*/
  ...
  private async getFilePath(): Promise<string> {
    if (!this.filePath) {
      const root = this.rootResolver
        ? this.rootResolver()
        : (await import("../electron-stub.ts")).app.getPath("userData");
...
export async function ensureUserDataDir(...segments: string[]): Promise<string> {
  const dir = path.join((await import("../electron-stub.ts")).app.getPath("userData"), ...segments);
