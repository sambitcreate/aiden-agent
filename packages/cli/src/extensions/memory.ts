 ...
 * (before_agent_start) instead of freezing a startup snapshot.
 */
...
import { legacyWorkspaceScopeIds, memoryEnabledFor, workspaceScopeId } from "./memory-scope.ts";
...
export async function createMemoryInlineExtension(options: {
	...
	const store = new MemoryStore({
		...
		imports: async () => [{
			...
			scopeIds: legacyWorkspaceScopeIds(options.workspaceRoot),
		}],
	});
	const scope = { kind: "workspace" as const, id: workspaceScopeId(options.workspaceRoot) };
	const enabled = async (): Promise<boolean> => memoryEnabledFor(options.agentDir);
	const extension = await createMemoryExtension({
		store,
		scope,
		provenance: { kind: "user_edit", sourceId: "aiden-cli" },
		enabled,
	});
