...
- **Design Workspace** - open Design from the persistent sidebar and work on a React Flow-powered infinite canvas of live, network-free HTML artboards and image references. React Grab can target the exact rendered element under the pointer, and the elevated composer refines that context with the current provider and model. The same [Design Workspace roadmap](docs/plans/design-workspace-plan.md) carries this shipped studio into explicit source-backed Vite/React preview, reviewed changes, undo, and later GitHub/IDE handoff without silently widening repository authority.
...
- **Source-backed Design Workspace** - generated HTML design ships today; local-project preview and write-back do not. The next gated track proves Vite/React containment and DOM-to-source identity before it can propose a bounded change, require approval, show the exact action diff, and offer hash-safe undo. See the unified [Design Workspace plan](docs/plans/design-workspace-plan.md).
