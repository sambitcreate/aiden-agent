...
3. The renderer parses the closed union fail-closed, then loads a **main-built** `srcdoc` over `chats:htmlArtifactSrcdoc`. The iframe is `sandbox="allow-scripts"` only. Guest CSP sets `connect-src 'none'`. Parent CSP `frame-src` is `'self'` so srcdoc frames work without opening arbitrary `https:` frames.
4. Host Chart.js, Plotly, and KaTeX load from the `aiden-genui:` protocol (exact library names). Export inlines those libraries into one offline `.html` file. There is no CDN.
...
- Keep large or seekable media out of Pi history. HTML bytes stay in the app-owned store and cross the renderer only as main-wrapped `srcdoc`, never as a workspace `file://` path.
