...
| [Telegram Remote Control](telegram-remote-control-plan.md)                                       | Planned | Port `pi-telegram`'s TypeScript adapter into an always-on main-process service backed by Aiden's existing headless-turn seam (`schedule-execution.ts`); full-unattended, single-owner, one persistent chat. |
