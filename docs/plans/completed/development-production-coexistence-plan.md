...
Status: Complete — implemented and independently reviewed 2026-07-29
...
- A pre-main bootstrap resolves the profile before Electron imports or the
  single-instance lock, with distinct production/development identities and
  roots.
- The development bundle, executable, Electron helpers, menu, page title,
  About surface, and Dock badge all use the `Aiden Agent Dev` identity.
- Development shortcuts remain opt-in and update checks remain
  production-only.
- Controlled side-by-side launches left the installed production app and its
  state untouched.
- Focused runtime/profile, wrapper-branding, update, and shortcut contracts;
  type-check; lint; production build; wrapper generation; and the full test
  suite passed.
- Two adversarial fix/review rounds and a wholly fresh runtime and packaging
  review pair returned clean.
