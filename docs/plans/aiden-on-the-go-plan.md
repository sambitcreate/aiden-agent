...
Attachment-picker follow-up 2026-09-20: iOS now uses Aiden's anchored Files/Camera/Photos surface instead of the system photo picker. The camera remains in-process with explicit authorization, rotation, camera switching, interruption recovery, and cancellation handling; PhotoKit selection preserves tap order and shares the existing ten-attachment capacity with pending files and images. The current chat draft-restoration, read-only, upload, streaming, Bot-vision, and attachment transport boundaries remain authoritative. The generic iOS test build and registered iOS release-policy suite pass; focused physical-device XCTest, camera, and iPad system-UI acceptance remain open.

Native recovery follow-up 2026-09-22: iOS and Android now cold-replay from sequence zero because partial response buffers are process-local, retain warm cursors in memory without per-event cache writes, preserve optimistic sends across refreshes, and hold Send until terminal replay/reconciliation settles. Android terminates transport on a terminal frame; transient recovery warnings clear on recovery. Focused native regressions and two independent review passes cover the changes. Generic iOS XCTest compilation and 177 Android JVM tests pass; physical iPhone XCTest remains blocked by the locked shared device. See `.memory/mobile-stream-recovery.md` for source pins and final PR validation.
...
Unified workspace sidebar follow-up 2026-08-30, updated 2026-09-01: iOS/iPadOS and Android share Electron's workspace-owned chat projection. The default home outline groups chats beneath persisted per-installation workspace disclosures, with a non-duplicating Recent-only alternative and explicit creation for empty workspaces. Archived and removed workspaces are excluded before projection. iPhone retains push navigation; regular-width iPad opens selected chats in the detail column. Compatible Mac builds now advertise a bounded, transcript-free paginated summary read for native home loaders; older builds retain the existing full-chat fallback, and opening any chat still fetches its authoritative full detail.
