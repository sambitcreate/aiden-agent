...
Unified workspace sidebar follow-up 2026-08-30: iOS/iPadOS and Android now share Electron's workspace-owned chat projection. The default home outline groups chats beneath persisted per-installation workspace disclosures, with a non-duplicating Recent-only alternative and explicit creation for empty workspaces. Archived and removed workspaces are excluded before projection. iPhone retains push navigation; regular-width iPad opens selected chats in the detail column. The existing full-chat home read remains functionally unchanged, with a paginated summary-only read documented as a measured follow-up in `unified-workspace-sidebar-plan.md`.
