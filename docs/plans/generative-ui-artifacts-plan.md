...
- **Visualizing shimmer:** a `ReasoningBlock` reuse (`label="Visualizing"`) rides above the artifact list whenever a `render_artifact` step is pending/awaiting/running and owns that phase instead of duplicating it in the bottom activity row.
- **Provider-neutral live activity:** `llm-client` consumes `toolcall_start` to open a pending timeline step while the model is still writing tool-call arguments (the longest phase of a GenUI turn); execution events upgrade it with the full descriptor. The activity row keys **Responding…** off recent text deltas (`TEXT_STREAMING_IDLE_MS`), while exposed provider reasoning is represented by one disclosure that changes from shimmering **Thinking** to settled **Thought briefly/for …** and suppresses duplicate thinking timeline and orb rows. Providers without exposed reasoning retain the generic **Thinking** shimmer.
