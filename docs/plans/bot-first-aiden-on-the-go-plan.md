...
Implementation completed August 23, 2026. Mobile rollout now fails closed through capability advertisement and every Bot surface ingress; the exact-pairing logo coachmark introduces Bots/Workspaces and the Full/Custom decision without widening read-only grants. Existing Mac Bot and shared-chat surfaces render path-private canonical photos over semantic fallbacks through a visibility-gated, four-request, 32 MiB cache. Telegram's shared protected resolver and stricter surface ceiling are regression-locked, desktop onboarding and internal TestFlight build 17 metadata are accurate, the complete repository gate is green, and 27/27 final selected tests pass on the connected physical iPhone 13 Pro without a simulator. Independent review closed every actionable P0/P1 finding. The plan remains Active for the recorded eligible-hardware, physical-iPad, multi-device/Mac, packaged update/rollback, live Telegram, wider staged TestFlight, Xcode 27, accessibility, and App Store owner gates. Evidence: `docs/testing/aiden-on-the-go/bot-first-phase-8.md`.
...
Completed August 23, 2026. Every Bot now owns exactly one writable persistent chat. Remote open-or-create is serialized per Bot and converges concurrent requests and retries on the same canonical chat; older duplicate records remain readable for recovery but are hidden and read-only. iOS resumes that chat from favorites, inbox, search, profile, and the bottom dock. Bot data is cache-first with stale-while-revalidate segment merging, optimistic favorite updates, connection reuse, and layout-shaped reduced-motion-aware skeletons. Selecting a Bot now pushes the chat chrome immediately, admits only an exact installation-scoped cached Bot chat, and resolves fresh authority in the background; a true cache miss shows a Bot-header, bubble, and composer skeleton instead of a blank transition. Custom Access now distinguishes revision drift from transport/server failures, hides unusable unselected skill tombstones, and preserves retryable drafts. New/Edit Bot exclusively own the Bot's revisioned, durable provider/model authority; the persistent chat mirrors it through crash-recoverable lifecycle reconciliation without replacing history or widening reduced grants, while Bot composers and access-reduction sheets expose no competing model selector. Obsolete provider/model failures produce actionable inline recovery instead of duplicate modal alerts. Internal TestFlight build 20 is `VALID` and `IN_BETA_TESTING` for Internal Testers. Its shipped source includes atomic desktop creation, fresh-inventory transaction retries, conflict-safe Mac/iOS edit rebasing, the contact-first Messages-inspired flow, stable avatar caching, and immediate cache-first Bot chat entry. Evidence: `docs/testing/aiden-on-the-go/bot-first-phase-9.md`.

Final-reply follow-up completed August 24, 2026. Bot conversations now present only the post-tool final answer as the assistant bubble. Intermediate assistant narration and tool activity remain available in the existing collapsed activity disclosure, with repeated progress paragraphs deduplicated. Projection resets clear the Bot's ephemeral cumulative stream before the replacement snapshot is applied, preventing replayed text from being appended to itself. Workspace conversations retain their existing transcript behavior. Internal TestFlight build 21 carries the fix and is `VALID` / `IN_BETA_TESTING` for Internal Testers.
...
4. Make New/Edit Bot the only provider/model controls, persist that choice on the one Bot chat, keep Workspace composer selection unchanged, improve stale-model recovery, and remove duplicate blocking error presentation.
...
## 2026-09-22 — Hermex cockpit follow-on

Current-main parity audit confirms task progress and bounded read-only agent inspection already ship on both native clients. The pending-request/Stop hardening slice binds decisions to the displayed request, suppresses duplicate writes, reconciles uncertain answers from the Mac, gates Bot mutations by current grants, preserves older-server Workspace Stop, and prevents delayed control responses from undoing terminal state. Android's control HTTP writes explicitly disable automatic connection retries. Native focused tests and independent blast-radius/adversarial reviews accompany the change; physical iOS execution is still blocked by the locked test device.

Queue/Steer is **unfinished assigned work**, dependent on the runtime-owned admission/persistence contract. Proposed controls must identify the exact stream and request, distinguish accepted input from model consumption, preserve uncertain drafts, and never turn a busy-to-idle race into an implicit Send. Do not expose controls or claim parity before the host advertises the supported contract. Mobile clarification/secret prompts and subagent interrupts likewise have no current Aiden Remote contract; existing bounded read-only inspection is retained.

### Queue/Steer contract and native implementation

PR #220 supplies the host-owned admission contract. The native follow-on adds feature-gated plain-text Steer/Queue to both shared Bot composers, exact stream/request receipt checks, and restart-safe unconfirmed-submission markers. Admission copy explicitly distinguishes queuing from model consumption. Unknown outcomes never become automatic retries or ordinary Send; rejected receipts retain the draft; accepted receipts preserve newer edits and require successful durable draft cleanup before dropping uncertainty. Onboarding and the existing Bots gallery describe the supported controls without a new transport or authority.

Implementation and independent reviews are complete locally; the native PR/host dependency and CI still require program integration. Physical iOS XCTest remains blocked by the locked device. Pending questions, secret prompts, and child interrupts remain unsupported pending their own host contract; they are not advertised.
