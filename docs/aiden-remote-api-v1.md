...
- `GET /bots/{botId}`: authenticated detail with editable guidance, a safe access view, and the Bot's optional current audience-safe provider/model selection from its revisioned policy authority. Private runtime provider/model identities never cross this boundary.
...
- `PATCH /bots/{botId}/capabilities`: `If-Match` Full/Custom update with the exact current `catalogRevision`. Full requires `confirmedForeground: true` and may carry one exact provider/model pair; omitting both preserves the saved choice for compatibility with older clients, while supplying only one is invalid. Custom contains exactly one currently available provider/model pair plus only exact positive opaque selections from that catalog. A supplied available pair becomes revisioned, durable Bot model authority; the canonical persistent chat is a recoverable execution mirror, and any Custom chat reduction keeps its non-model grants. An unavailable model remains visibly blocked and is never replaced by fallback.
