...
Generation timeline thinking steps may carry optional UTF-16 `reasoningStartOffset` and `reasoningEndOffset` fields into the separately projected readable reasoning string. These offsets let clients render multiple chronological reasoning disclosures around prose and tool milestones; older steps without them retain the legacy single-disclosure presentation.
