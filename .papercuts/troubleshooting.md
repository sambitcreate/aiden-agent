...
- `npm run format` (oxfmt) is not a safe fix-up step on this checkout: several transcript files are oxfmt-dirty at HEAD, so a blanket format pass rewrites unrelated lines and can grow a PR diff by ~900 lines. Verify a file's formatting status at HEAD before running oxfmt, and restore + re-apply logical edits instead of accepting the whole-file rewrite.

## 2026-09-20 — Linux GUI testing of Electron app

- `npm run dev` is macOS-only (`prepare-macos-dev-runtime.mjs` throws "branded development runtime is available only on macOS"). Linux recipe: `node node_modules/electron/install.js` (blueprint's `npm install --ignore-scripts` skips Electron's postinstall, leaving no dist binary), then `node scripts/patch-pi-oauth-branding.mjs && node scripts/build-electron.mjs`, `npx vite --host 127.0.0.1 --port 4143 --strictPort`, then `DISPLAY=:0 AIDEN_RUNTIME_PROFILE=development AIDEN_RENDERER_URL=http://127.0.0.1:4143 ./node_modules/.bin/electron . --no-sandbox`.
- Startup requires `build/native/aiden-subagent-run-store`, a macOS-only C helper (`SubagentRunStoreStorageError` without it). A throwaway Linux port needs st_mtimespec→st_mtim, st_ctimespec→st_ctim, st_birthtimespec→st_mtim (NOT st_ctim — ctime changes on rename and breaks the helper's file-identity checks → "storage changed and was preserved"), renameatx_np(RENAME_EXCL)→renameat2(RENAME_NOREPLACE), RENAME_SWAP→RENAME_EXCHANGE, plus an arc4random_buf→getrandom shim.
- 2026-09-20 PR187 testing: `providers:save` fails on Linux without a keyring — `saveProviderWithCredentialRotation` journals even `needsKey:false` providers through `secrets.setInternalKey` → `safeStorage.isEncryptionAvailable()` false → "Secure storage is unavailable". Workaround for tests: close the app and seed `config.json` + `provider-model-cache.json` + `settings.json` on disk.
- 2026-09-20 PR187 testing: `AIDEN_RUNTIME_PROFILE=development` reads config from `~/.aiden-dev` (not `~/.aiden`) — `runtime-profile-core.ts` `configDirectory()`. Seeding `~/.aiden/config.json` is silently ignored; chat files live under `~/.config/Aiden Agent Dev/chats/`.
- 2026-09-20 PR187 testing: `build/main/index.js` is NOT rebuilt when Electron launches — only `build-electron.mjs` regenerates it. A stale main bundle vs vite-served renderer caused `chats:contextPressure` "Invalid context pressure request" (old `CONTEXT_PRESSURE_KEYS` missing providerId/modelId/attachments; `exactRecord` rejects unknown keys). Rebuild after every checkout/rebase before GUI testing.
- 2026-09-20 PR187 testing: typing ~40-60KB into the composer via synthetic `type` actions (per-keystroke) crashed/reloaded the renderer twice → app fell back to the onboarding overlay with "Onboarding state is unavailable outside the active application document". Chat data persisted. Use `keyboard.insertText` (single paste-style input event) for large drafts instead — it exercises the same debounced draft path without the event flood.
...
- 2026-09-26 PR #187: `scripts/verify-macos-package.test.mjs` ("package verifier requires the local VCC worker and pinned attribution") failed once inside a full `npm run test` with "Packaged VCC attribution is missing." It passed when rerun alone. Treat it as a full-suite flake and rerun the file alone before investigating.
...
- 2026-09-26 PR #187 pi 0.87.1 merge: after merging a Pi bump, `node_modules` still holds the old Pi until `npm install`, so API probes and typecheck read the stale 0.84 types. Install before resolving conflicts that depend on the new API.
- 2026-09-26 PR #187 round 4: fixing one estimate path (whole-transcript) left the anchored terms with the same system-message gap, because Pi's anchor estimate and `messageTokens` price system messages at zero. Grep every `estimateContextTokens`/`estimateTokens` caller (generation-context and pi-compaction-core) when changing what the transcript sends.
- 2026-09-26 PR #187 round 5: in the compaction prefix ratio `(usage - static) / prefix`, `static` cancels out of `static + prefix*ratio`, so a static-prompt adjustment there needs the anchor-time static, not the tail. Probe the candidate totals directly before trusting a symmetric fix.
