...
- 2026-09-26 (PR217 iOS fixes): stacked ownership PRs (#233-#238) were only compiled, never executed, so their regressions against #217's cache tests surfaced only after the first simulator run. Execute the scoped XCTests with the device workflow in `ios/AGENTS.md` before merging a stacked iOS cache change. (This session used the iPhone 17 Pro simulator only because the owner explicitly asked for that here; it is not standing guidance.) A `test-without-building` run right after a killed run often fails with "failed to launch ... No such process"; rerunning once succeeds.
