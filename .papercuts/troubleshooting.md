...
- Fresh-VM sessions may lack node/npm on PATH and ship no `node_modules/`. Add `~/.nvm/versions/node/v24.19.0/bin` to PATH and `npm ci --ignore-scripts` before running focused `tsx --test` suites; Electron postinstalls are skippable for renderer contract tests.
- A `doesNotMatch` contract on `detachedGenerationDraining\s+\?` over-matches: the flag legitimately feeds a JSX ternary (`? displayedStreamingText`). Pin readiness-string assertions to `detachedGenerationDraining\s*\?\s*"` (ternary producing a string literal) instead.
