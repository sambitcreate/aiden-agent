...
- macOS reaches `os.tmpdir()` through `/var` -> `/private/var`. Containment checks that compare `realpath` results against an un-canonicalized root silently skip every file (the snapshot payload copied nothing); canonicalize both sides, and have temp-dir test helpers return `fs.realpath(...)`.
- Node's `execFile` timeout/maxBuffer kill only the direct child: a setup script's background descendants keep running and can hold the pipes open past success. Spawn setup detached and SIGKILL the whole process group before settling.
- The worktree git-deletion tests need `npm run build:worktree-remover` locally first; without the native helper every real deletion fails with "could not safely remove the managed worktree quarantine".
