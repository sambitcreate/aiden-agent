...
# Production bot/subagent investigation — 2026-09-21

- This detached worktree had no `node_modules`; focused suites require `npm ci` before running. The diagnostic journal deliberately records only broad error categories, so the managed-home failure required read-only reconciliation of the receipt, manifest, and live filesystem identity.
- `npm run test:bots` selected the Command Line Tools macOS 27 SDK despite Xcode 26.6 being selected; its `libSystem.tbd` is rejected by that linker. The native pretest script intentionally replaces the child environment (including `SDKROOT`/`DEVELOPER_DIR`), so an outer SDK pin cannot fix it. Run the registered TypeScript bot suite with npm lifecycle scripts suppressed; report the native pretest gap separately.
- PR #205 review exposed a time-of-check gap: checking root/homes devices on operation entry and then comparing home only to a fresh homes device can admit a swapped homes mount. Capture the entry root device and recheck all anchors at incarnation capture; use a deterministic device seam to test cross-device guards without privileged mounts.
- PR #205 Pullfrog review caught interrupted provisioning after remount: a stale durable receipt and live home token must not produce a new manifest with the live device. Keep the manifest aligned to the receipt and return the live token separately; cover reconciliation, audit, and revalidation.
