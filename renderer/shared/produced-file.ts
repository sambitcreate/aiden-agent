...
export function parseProducedFile(value: unknown, toolName: string): ProducedFile | undefined {
  ...
  if (!expected || file.operation !== expected || typeof file.relativePath !== "string" ||
    !file.relativePath || file.relativePath.length > 480 || Array.from(file.relativePath).length > 240 ||
    Array.from(file.relativePath).some((character) => character === "\\" || character.charCodeAt(0) < 32 || character.charCodeAt(0) === 127) || /^[~/]/u.test(file.relativePath) || /^[A-Za-z]:\//u.test(file.relativePath) ||
