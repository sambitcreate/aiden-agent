...
export const DropdownMenuCheckboxItem = React.forwardRef<React.ElementRef<typeof DropdownMenuPrimitive.CheckboxItem>, React.ComponentPropsWithoutRef<typeof DropdownMenuPrimitive.CheckboxItem> & { sublabel?: string }>(function CheckboxItem({ className, children, sublabel, ...props }, ref) { return <DropdownMenuPrimitive.CheckboxItem ref={ref} className={cn(menuItemClass, "pl-7", className)} {...props}><span className="absolute left-2"><DropdownMenuPrimitive.ItemIndicator><Check className="size-3.5" /></DropdownMenuPrimitive.ItemIndicator></span><span className="flex min-w-0 flex-col"><span>{children}</span>{sublabel ? <span className="max-w-72 truncate text-small text-tertiary">{sublabel}</span> : null}</span></DropdownMenuPrimitive.CheckboxItem>; });
