...
const idleLive: AssistantLiveController = {
  ...
  microphonePermissionDetail: "Microphone permission is allowed in macOS System Settings.",
  ...
  computerUseDetail: "Turn on the Computer Use beta to make it available in individual chats.",
...
test("composer Live entry stays truthful for reconnect and blocked states", () => {
  ...
  for (const reason of unavailableReasons) {
    ...
    assert.match(html, new RegExp(reason.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
...
test("Live setup surfaces blocked and failed starts inside the consent UI", () => {
  ...
  const failed = renderToStaticMarkup(
    <AssistantLiveSetupContent live={{ ...idleLive, error: "Live could not start. Try again." }} />,
  );
...
test("active Live setup reports microphone startup instead of claiming capture is off", () => {
  ...
  assert.match(html, /role="status" aria-live="polite"[^>]*>.*Starting microphone…/u);
...
test("Live start and setup confirmation expose the exact ordinary-response collision", () => {
  for (const reason of [
    ...
    const setup = renderToStaticMarkup(
      <AssistantLiveSetupContent live={{ ...idleLive, startBlockedReason: reason }} />,
    );
    assert.match(setup, new RegExp(reason.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
    const entry = renderToStaticMarkup(
      <AssistantLiveEntryPoint live={{ ...idleLive, startBlockedReason: reason }} />,
    );
...
test("mounted dock ignores minimize timers while Live is active and keeps Stop mounted", async () => {
  ...
  try {
    ...
    flushSync(() => undefined);
    assert.equal(timers.size, 0, "blocked minimize must not schedule the exit timer");
...
test("New conversation explains the Live ownership block", () => {
  ...
  assert.match(html, /New conversation unavailable: Stop Live before changing conversations/u);
...
test("Live Computer Use confirmation is exact-action Allow once with Deny first", () => {
  ...
  assert.ok(
    html.indexOf("Deny Computer Use action") < html.indexOf("Allow this Computer Use action once"),
  );
});
