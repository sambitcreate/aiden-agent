...
final class AidenChatTests: XCTestCase {
    ...
    private func exerciseControlResponse(stop: Bool, nextApproval: String, mode: AidenChatProgressLifecycleURLProtocol.Mode = .controls) async throws {
        ...
        if mode == .mismatchedApproval, let coordinator {
            // #217 keeps reopen from re-probing a live stream, so deliver the
            // Mac's newer request through the authoritative snapshot read.
            await model.restorePendingApproval(
                streamID: "stream-control",
                context: try coordinator.requestContext(for: "instance-progress-lifecycle")
            )
