...
- Use terminal-based `xcodebuild`/`xcrun` verification against an explicitly selected simulator or physical device. Simulator builds and XCTest execution are authorized for development and regression verification. Record the destination and distinguish simulator results from physical-device acceptance. Use a connected, unlocked physical device for hardware-dependent behavior (including real audio routes and interruption handling), manual Keychain/entitlements checks, and release acceptance; an unsupported device proves the Image Playground-unavailable fallback, not a successful Apple Intelligence generation flow. A `CODE_SIGNING_ALLOWED=NO` build is compile/test-only and is not valid manual Keychain or entitlements evidence.
