...
- Use terminal-based `xcodebuild`/`xcrun` verification against an explicitly selected physical device or simulator. The owner permits simulator testing. Use available simulators for automated tests and UI checks; retain physical-device verification for hardware-specific behavior, real audio routes/interruption handling, and signing, Keychain, or entitlements acceptance. Use an available, unlocked paired iPhone for physical tests and record the actual destination. An iPhone 13 Pro can prove the Image Playground-unavailable fallback, not successful Apple Intelligence generation. Report simulator and physical-device evidence separately. A `CODE_SIGNING_ALLOWED=NO` build is compile/test-only and is not valid manual Keychain or entitlements evidence.
...
- Remove Kanban and every Hermes-only panel, profile/personality concept, generic voice-note upload path, and Cloudflare-specific onboarding during the designated cleanup phases. Permitted remote speech paths are explicit composer dictation to the paired Mac's local model and the user-approved desktop-configured Read Aloud `tts-v1` contract. Mobile TTS settings remain read-only; credentials stay on the Mac.
- Workspace permission belongs in Workspace Settings opened from the conversation-toolbar ellipsis, never in the composer.
- App Intents are App-Group-cache-only navigation. Live Activities are bounded last-known state without cloud push. Dictation may use native on-device recognition or the explicitly selected paired-Mac local model; Read Aloud uses native playback with the desktop-owned `tts-v1` setup; mobile enablement/configuration is deferred.
