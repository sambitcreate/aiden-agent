...
class TestFlightBuildNumberSelectorTest < Minitest::Test
  ...
  def test_rejects_more_than_three_build_number_components
    error = assert_raises(TestFlightBuildNumberSelector::SelectionError) do
      TestFlightBuildNumberSelector.select_build_number(
        requested_build_number: "1.2.3.4",
        latest_build_number: "1.2.3"
      )
    end

    assert_includes(error.message, "one to three")
  end

  def test_rejects_empty_build_number_components
    [".1", "1.", "1..2"].each do |value|
      assert_raises(TestFlightBuildNumberSelector::SelectionError) do
        TestFlightBuildNumberSelector.select_build_number(
          requested_build_number: value,
          latest_build_number: nil
        )
      end
    end
  ...
  def test_comparison_treats_omitted_trailing_zero_components_as_equal
    assert_equal(0, TestFlightBuildNumberSelector.compare_build_numbers("1", "1.0"))
    assert_equal(0, TestFlightBuildNumberSelector.compare_build_numbers("1.0", "1.0.0"))
  end
  ...
  def test_approved_versions_fetch_filters_to_approved_states
    ...
    versions = selector.send(:approved_app_store_versions, bundle_id: "sbtbiswas.AidenOnTheGo")
  ...
  def test_enforce_open_train_raises_with_actionable_message_when_closed
    ...
    error = assert_raises(TestFlightBuildNumberSelector::SelectionError) do
      selector.send(:enforce_open_train!, bundle_id: "sbtbiswas.AidenOnTheGo", marketing_version: "1.4")
    end
  ...
  def test_app_lookup_uses_supported_bundle_id_filter_only
      ...
      [
        {
          "id" => "app-123",
          "attributes" => {
            "bundleId" => "sbtbiswas.AidenOnTheGo"
          }
        }
      ]
    end
    ...
    assert_equal(
      "app-123",
      selector.send(:app_id_for_bundle_id, "sbtbiswas.AidenOnTheGo")
    )

    assert_equal("/v1/apps", captured.first.first)
    assert_equal("sbtbiswas.AidenOnTheGo", captured.first.last.fetch("filter[bundleId]"))
