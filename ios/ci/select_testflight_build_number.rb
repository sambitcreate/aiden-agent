...
class TestFlightBuildNumberSelector
  ...
  BUILD_NUMBER_PATTERN = /\A\d+(?:\.\d+){0,2}\z/
  ...
  def self.validate_build_number!(value, label)
    ...
    raise SelectionError, "#{label.capitalize} must contain one to three period-separated integers using only digits and dots. Received: #{value}"
  end
  ...
  def enforce_open_train!(bundle_id:, marketing_version:)
    ...
    if blocking
      raise SelectionError,
            ...
            "Bump MARKETING_VERSION in ios/AidenOnTheGo.xcodeproj/project.pbxproj above #{blocking}, land it on main, " \
            "and re-run this workflow (see ios/TESTFLIGHT.md)."
    end
