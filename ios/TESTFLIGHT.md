...
On 2026-08-20, version `0.1.0` build `8` was distribution-signed, exported with the internal-only policy, uploaded through the explicitly selected owner-authorized ASC profile, processed as `VALID`, and assigned to the internal group. Exact App Store Connect build `b0d3c1c0-ab61-469f-9e4f-9dd250e1ff1a` reports `internalBuildState=IN_BETA_TESTING` and `externalBuildState=NOT_APPLICABLE`. Build `7` added Thinking Orbs and the refreshed composer/model controls; build `8` adds Mac-owned model visibility, sanitized custom provider artwork sync, corrected iOS thinking-model rows, waveform-only listening status, and the home action overlay. Build `1` was rejected before processing because its App Store icon contained alpha; builds `2`–`8` use the exact opaque RayChat Icon Composer artwork.
...
1. The owner selected version `0.1.0` for the first internal TestFlight train. Build `1` was rejected during upload because the App Store icon contained alpha; builds `2`–`7` incrementally repaired the icon, compact navigation, native shell, cold-connect loading, scratch/activity branding, and interaction/model UI. Build `8` is the current internal candidate with model visibility and custom provider identity sync. Increment the build number for later uploads; selecting the first public version remains a later product decision.
