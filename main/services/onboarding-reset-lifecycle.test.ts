...
test("the onboarding reset closes the renderer before destructive cleanup and preserves unrelated local storage", async () => {
  ...
  assert.match(reset, /shutdownStarted \|\| installUpdateOnQuit/u);
  const authorize = reset.indexOf('authorizeProtectedAction(window, "close")');
  const clearCompletion = reset.indexOf("await clearRendererOnboardingCompletion(window)");
  ...
  assert.ok(closeRenderer > clearCompletion, "renderer close begins after its completion marker is cleared");
  assert.ok(clearData > closeRenderer, "persistent setup is untouched until the renderer has closed");
  assert.ok(relaunch > clearData, "relaunch is armed only after destructive cleanup succeeds");
  assert.ok(shutdown > relaunch, "normal service shutdown runs after relaunch is armed");
  assert.doesNotMatch(reset, /clearStorageData/u, "unrelated renderer preferences are preserved");
  ...
  assert.match(reset, /await createMainWindow\(\)/u, "a failed post-close reset reopens the app");
});
