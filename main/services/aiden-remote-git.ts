...
export type AidenRemoteGitProjection =
  ...
  | {
      ...
      base: string;
      head: string;
      files: AidenRemoteGitFile[];
    }
  | { kind: "push-capability"; allowed: boolean; reason?: string; remote?: string; branch?: string }
  | {
      kind: "worktrees";
      worktrees: Array<{ id: string; name: string; branch: string; managed: boolean }>;
    }
  | {
      kind: "mutation";
      message: string;
...
interface GitDependencies {
  ...
  diff(
    ...
    input: GitDiffInput,
    signal?: AbortSignal,
  ): Promise<{ path: string; patch: string; truncated: boolean }>;
  ...
  commit(
    ...
    signal?: AbortSignal,
  ): Promise<{ commit: string; branch: string; subject: string; warning?: string }>;
  pushCapability(folderPath: string, signal?: AbortSignal): Promise<GitPushCapability>;
  push(
    folderPath: string,
    input: GitPushInput,
    signal?: AbortSignal,
  ): Promise<{ branch: string; commit: string; remote: string; warning?: string }>;
...
function safeDisplayPath(value: string): string {
  if (
    ...
    throw new AidenRemoteServiceError(
      ...
      "A Git path could not be projected safely.",
      409,
    );
  }
  return value;
...
function recordKeysWithin(record: Record<string, unknown>, allowed: string[]): boolean {
...
function mapGitError(error: unknown): never {
  ...
  if (!(error instanceof GitServiceError)) {
    throw new AidenRemoteServiceError(
      "internal_error",
      "Aiden could not complete this Git operation.",
      500,
    );
  }
  if (
    ...
    throw new AidenRemoteServiceError(
      "operation_stale",
      "The repository changed. Refresh Git and try again.",
      409,
    );
  }
...
export class AidenRemoteGitService {
  ...
  private issueSnapshot(
    ...
    if (this.snapshots.size >= MAX_SNAPSHOTS) {
      throw new AidenRemoteServiceError(
        "handle_capacity",
        "Aiden's Git snapshot capacity is temporarily full.",
        429,
        true,
      );
    }
  ...
  private snapshot(
    ...
    if (!snapshot)
      throw new AidenRemoteServiceError(
        "handle_expired",
        "This Git snapshot expired. Refresh Git and try again.",
        410,
      );
    if (snapshot.deviceId !== deviceId)
      throw new AidenRemoteServiceError(
        "handle_wrong_device",
        "This Git snapshot belongs to another paired device.",
        403,
      );
    if (snapshot.workspaceId !== workspaceId || snapshot.kind !== expectedKind) {
      throw new AidenRemoteServiceError(
        "operation_stale",
        "This Git snapshot does not match the workspace operation.",
        409,
      );
    }
    return snapshot;
  ...
  private filePath(snapshot: Snapshot, fileId: string): string {
    ...
    if (!value)
      throw new AidenRemoteServiceError(
        "operation_stale",
        "This file is not part of the Git snapshot.",
        409,
      );
    return value;
  }
  ...
  private async executeIdempotent<T>(
    ...
    try {
      ...
      throw new AidenRemoteServiceError(
        "internal_error",
        "Aiden could not durably prepare this Git operation.",
        500,
      );
    }
  ...
  async diff(deviceId: string, workspaceId: string, value: unknown): Promise<AidenRemoteGitResult> {
    ...
    if (
      !record ||
      !exactKeys(record, ["snapshotId", "fileId"]) ||
      !boundedString(record.snapshotId, 128) ||
      !boundedString(record.fileId, 128)
    ) {
    ...
    if (snapshot.kind !== "review" || !snapshot.expectedSnapshot) {
      throw new AidenRemoteServiceError(
        "git_capability_denied",
        "A diff is unavailable for this review.",
        409,
      );
    }
    ...
    try {
      const diff = await this.run(deviceId, workspaceId, (folderPath, signal) =>
        this.options.git.diff(
          folderPath,
          { expectedSnapshot: snapshot.expectedSnapshot!, path: displayPath },
          signal,
        ),
      );
      return {
  ...
  async checkout(
    ...
    if (
      !record ||
      !exactKeys(record, ["branch", "snapshotId", "confirmedForeground"]) ||
      !boundedString(record.branch, 500) ||
      !boundedString(record.snapshotId, 128)
    ) {
      throw new AidenRemoteServiceError(
        "invalid_request",
        "The branch checkout request is invalid.",
        400,
      );
    }
        ...
        try {
          return await this.run(deviceId, workspaceId, async (folderPath, signal) => {
            ...
            if (snapshot.kind !== "branches" || snapshot.digest !== branchesDigest(current)) {
              throw new AidenRemoteServiceError(
                "operation_stale",
                "The branch list changed. Refresh Git and try again.",
                409,
              );
            }
  ...
  async createBranch(
    ...
    if (
      !record ||
      !exactKeys(record, ["name", "startPoint", "confirmedForeground"]) ||
      !boundedString(record.name, 200) ||
      !boundedString(record.startPoint, 500)
    ) {
      throw new AidenRemoteServiceError(
        "invalid_request",
        "The branch creation request is invalid.",
        400,
      );
    }
    requireConfirmation(record);
        ...
        try {
          return await this.run(deviceId, workspaceId, async (folderPath, signal) => {
            ...
            if (record.startPoint !== (branches.current ?? "HEAD")) {
              throw new AidenRemoteServiceError(
                "operation_stale",
                "The branch start point changed. Refresh Git and try again.",
                409,
              );
            }
            ...
            return {
              ...
              result: {
                kind: "mutation" as const,
                message: "Created and checked out branch.",
                branch: record.name as string,
              },
            };
          });
        } catch (error) {
  ...
  async commit(
    ...
    if (
      ...
      throw new AidenRemoteServiceError(
        "invalid_request",
        "The Git commit request is invalid.",
        400,
      );
    }
    requireConfirmation(record);
    ...
    if (snapshot.kind !== "review" || !snapshot.expectedSnapshot) {
      throw new AidenRemoteServiceError(
        "git_capability_denied",
        "Commit is unavailable for this review.",
        409,
      );
    }
    return this.executeIdempotent(
    ...
    try {
      ...
      return {
        ...
        result: {
          ...
          ...(capability.destinationBranch
            ? { branch: safeString(capability.destinationBranch, 500) }
            : {}),
        },
      };
    } catch (error) {
  ...
  async push(
    ...
    if (
      !record ||
      !exactKeys(record, ["snapshotId", "remote", "branch", "confirmedForeground"]) ||
      !boundedString(record.snapshotId, 128) ||
      !boundedString(record.remote, 200) ||
      !boundedString(record.branch, 500)
    ) {
    ...
    if (snapshot.kind !== "push" || !snapshot.capability.allowed) {
      throw new AidenRemoteServiceError(
        "git_capability_denied",
        "Push is not available for this workspace state.",
        409,
      );
    }
    ...
    if (
      ...
      throw new AidenRemoteServiceError(
        "operation_stale",
        "The reviewed push destination changed. Refresh Git and try again.",
        409,
      );
    }
    return this.executeIdempotent(
      ...
      async () => {
        try {
          ...
          const result = await this.run(deviceId, workspaceId, (folderPath, signal) =>
            this.options.git.push(folderPath, input, signal),
          );
          return {
            operationId: operationId(),
            status: "succeeded" as const,
  ...
  async compare(
    ...
    if (!record || !exactKeys(record, ["baseRef"]) || !boundedString(record.baseRef, 500)) {
      throw new AidenRemoteServiceError(
        "invalid_request",
        "The Git comparison request is invalid.",
        400,
      );
    }
    try {
      const comparison = await this.run(deviceId, workspaceId, (folderPath, signal) =>
        this.options.git.compare(folderPath, record.baseRef as string, signal),
      );
      const issued = this.issueSnapshot(
  ...
  async comparisonDiff(
    ...
    if (
      !record ||
      !exactKeys(record, ["comparisonId", "fileId"]) ||
      !boundedString(record.comparisonId, 128) ||
      !boundedString(record.fileId, 128)
    ) {
      throw new AidenRemoteServiceError(
        "invalid_request",
        "The comparison diff request is invalid.",
        400,
      );
    }
    const snapshot = this.snapshot(deviceId, workspaceId, record.comparisonId, "comparison");
    if (snapshot.kind !== "comparison")
      throw new AidenRemoteServiceError("operation_stale", "This comparison is stale.", 409);
    const displayPath = this.filePath(snapshot, record.fileId);
    const comparison = snapshot.comparison;
    try {
      const diff = await this.run(deviceId, workspaceId, (folderPath, signal) =>
        this.options.git.comparisonDiff(
          folderPath,
          {
  ...
  async worktrees(deviceId: string, workspaceId: string): Promise<AidenRemoteGitResult> {
    try {
      ...
      const projected = worktrees.flatMap((worktree) => {
        const resolved = resolvedWorkspaces.find(
          (candidate) => candidate?.folderPath === worktree.path,
        );
        if (!resolved) return [];
        return [
          {
        ...
        try {
          ...
          return {
            ...
            result: {
              ...
              branch: safeString(
                workspace.managedWorktree?.branch ?? (record.branch as string),
                500,
              ),
  ...
  async deleteManagedWorktree(
    ...
    if (
      !record ||
      !recordKeysWithin(record, ["confirmedForeground", "force"]) ||
      (record.force !== undefined && typeof record.force !== "boolean")
    ) {
      throw new AidenRemoteServiceError(
        "invalid_request",
        "The managed-worktree deletion request is invalid.",
        400,
      );
    }
    requireConfirmation(record);
              ...
              ...(result.snapshot
                ? {
                    snapshot: {
                      ...
                      ...(result.snapshot.expiresAt !== undefined
