...
export const llmClient = {
  async start(
    ...
    if (
      !turnId ||
      !chatTurnAdmission.handoff(params.chatId, turnId, owner.documentId, () => {
    ...
    try {
      ...
      await syncChatMessagesToPiSession(piSession, priorVisibleMessages, model, supportsImages);
      ...
      const initialMessages = currentUser
        ? [...prePromptMessages, chatMessageToPiMessage(currentUser, model, supportsImages)]
    ...
    const flushPiMessages = async (): Promise<boolean> => {
      ...
      try {
        ...
        logger.error("pi", `Could not append Pi session messages for stream ${streamId}.`, error);
        return false;
      }
    };
    ...
    const runWithPiCompaction = async () => {
      for (;;) {
        ...
        const completedAssistant = lastAssistantMessage as AssistantMessage | undefined;
