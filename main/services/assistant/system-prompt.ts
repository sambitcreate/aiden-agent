...
export function withTelegramAgentContract(prompt: string, input: TelegramAgentPromptInput): string {
  ...
  return [
    ...
    "The host can receive text, photos, text documents, private local copies of general files,",
    "transcribed voice messages, replies, and forwarded-message context. It delivers your final",
    "response back to Telegram using native Rich Markdown when available.",
    ...workspaceCapability,
    ...
    "You can call telegram_help to inspect paired profiles and live thread targets. Use",
    "telegram_message, telegram_attach, or telegram_voice only for an explicit direct-delivery",
    "request; ordinary replies are delivered by the host and must not be duplicated with a tool.",
