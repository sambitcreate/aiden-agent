...
export function createLocalModelManager(dependencies: {
  root(): string;
  progress?(value: { id: string; downloaded: number; total: number; percentage: number; phase: "download" | "extract" }): void;
  info?(message: string): void;
}) {
function modelsRoot(): string {
  const dir = dependencies.root();
...
async function performDownload(id: string): Promise<void> {
  ...
  try {
    ...
    dependencies.info?.(`Installed Parakeet model "${id}"`);
  } catch (error) {
...
function downloadModel(id: string): Promise<void> {
  ...
  void operation.then(() => pendingDownloads.delete(operation), () => pendingDownloads.delete(operation));
  return operation;
}
...
return { modelDir, isModelInstalled, listModels, localModelDownloadStates, downloadModel, cancelDownload, deleteModel, stopDownloads };
}
