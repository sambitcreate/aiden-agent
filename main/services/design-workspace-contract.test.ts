...
test("damaged generated bytes cannot cross code, export, edit, comment, or handoff boundaries", () => {
  ...
  const authority = readFileSync(new URL("./design-handoff-packet-authority.ts", import.meta.url), "utf8");
  assert.match(handoff, /buildDesignHandoffContext/u);
  assert.match(llm, /buildDesignHandoffContext/u);
  assert.match(authority, /isUsablePublishedDesignSource\(project, source\)/u);
