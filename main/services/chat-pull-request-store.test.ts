...
test("deletion waits for a read that is recovering a held PR file", async (t) => {
  ...
  await fs.writeFile(
    path.join(
      directory,
      ".chat-1.json.absent.00000000-0000-4000-8000-000000000000.held",
    ),
    JSON.stringify({
      schemaVersion: 1,
      chatId: "chat-1",
...
test("deletion consumes failed recovery artifacts before restart without touching another chat", async (t) => {
  ...
  await fs.writeFile(
    path.join(
      directory,
      ".chat-1.json.absent.00000000-0000-4000-8000-000000000001.held",
    ),
    payload,
  );
  await fs.writeFile(
    path.join(
      directory,
      ".chat-1.json.absent.00000000-0000-4000-8000-000000000001.previous",
    ),
    payload,
  );
  await fs.writeFile(
    path.join(
      directory,
      ".chat-10.json.absent.00000000-0000-4000-8000-000000000002.held",
    ),
    "other chat",
  );
  const prototype = DataStore.prototype as unknown as {
...
test("deletion consumes recovery artifacts even when no store was loaded", async (t) => {
  ...
  await fs.writeFile(
    path.join(
      directory,
      ".chat-1.json.absent.00000000-0000-4000-8000-000000000003.held",
    ),
    JSON.stringify({
      schemaVersion: 1,
      chatId: "chat-1",
  ...
  for (const cached of [false, true]) {
    test(`deleting a chat preserves dotted sibling recovery files (${deletedId}, cached=${cached})`, async (t) => {
      ...
      const siblingNames = ["held", "previous"].map(
        (suffix) =>
          `.${siblingId}.json.absent.00000000-0000-4000-8000-000000000004.${suffix}`,
      );
      ...
      await fs.writeFile(
        path.join(
          directory,
          `.${deletedId}.json.absent.00000000-0000-4000-8000-000000000005.held`,
        ),
        "own data",
      );
      await fs.writeFile(
        path.join(
          directory,
          `.${deletedId}.json.absent.00000000-0000-4000-8000-000000000005.previous`,
        ),
        "own data",
      );
...
test("recovery and deletion preserve files with non-generated operation tokens", async (t) => {
  const directory = await mkdtemp(path.join(tmpdir(), "aiden-chat-pr-"));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const names = [
    "notes",
    ...
    ["held", "previous"].map(
      (suffix) => `.chat-1.json.absent.${token}.${suffix}`,
    ),
  );
  const payload = JSON.stringify({
    schemaVersion: 1,
