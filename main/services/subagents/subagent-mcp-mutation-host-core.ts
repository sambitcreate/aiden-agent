import type { SubagentMcpReadHost } from "./subagent-mcp-read.js";
import {
...
export function createSubagentMcpMutationHost(readHost: SubagentMcpReadHost): SubagentMcpMutationHostV2 { return Object.freeze({
  async openFreshSession(
    binding: SubagentMcpMutationBindingV2,
    signal: AbortSignal,
  ): Promise<SubagentMcpMutationRemoteSessionV2> {
    if (signal.aborted) throw abortReason(signal);
    const server = await readHost.resolveServer(binding.serverId, signal);
