...
test("native Google streaming uses the google-generative-ai protocol", async (t) => {
  let requestPath = "";
  let requestBody = "";
  let apiKeyHeader = "";
    ...
    request.on("end", () => {
      ...
      response.end(
        `data: ${JSON.stringify({
          candidates: [
            {
              content: { role: "model", parts: [{ text: "Native Google response" }] },
              finishReason: "STOP",
            },
          ],
          usageMetadata: {
  ...
  const baseModel = service.getModel("gemini-2.0-flash");
  assert.ok(baseModel);
  const model = {
    ...baseModel,
  ...
  assert.match(requestPath, /\/v1beta\/models\/gemini-2\.0-flash:streamGenerateContent/u);
  assert.equal(apiKeyHeader, "native-test-key");
  assert.match(requestBody, /systemInstruction/u);
