...
export const buildBriefSections = (blocks: NormalizedBlock[]): BriefLine[] => {
  ...
  for (let blockIndex = 0; blockIndex < blocks.length; blockIndex++) {
    ...
    switch (b.kind) {
      case "user": {
        ...
        if (text) {
          const ref = b.sourceIndex != null ? ` (ref:${b.sourceIndex})` : "";
  ...
  // Aiden retains individual stable references. The upstream numeric-reference
  // coalescer is omitted; the per-turn cap below still bounds repetitive calls.
