...
const normalizeOne = (msg: Message | { role: "bashExecution"; command?: string; output?: string; exitCode?: number }, msgIndex: number | string): NormalizedBlock[] => {
  if (msg.role === "user") {
...
export const normalize = (messages: Message[], references?: string[]): NormalizedBlock[] =>
  messages.flatMap((msg, i) => normalizeOne(msg, references?.[i] ?? i));
